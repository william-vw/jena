...
/**
 * Checks a triple against the grounded matches and intra-triple matches for a
 * single rule clause. If the match passes it creates a binding environment
 * token and passes it on the RETE network itself. The checks and bindings are
 * implemented using a simple byte-coded interpreter.
 */
public class RETEClauseFilter implements RETESourceNode {
	 ...
	 * only usable after a successful TestFunctorName.
	 */
	 ...
	 * @param args         the object arguments referenced from the instructions
	 *                     array.
	 */
	...
	/**
	 * Create a filter node from a rule clause. Clause complexity is limited to less
	 * than 50 args in a Functor.
	 * 
	 * @param clause    the rule clause
	 * @param envLength the size of binding environment that should be created on
	 *                  successful matches
	...
	public static RETEClauseFilter compile(TriplePattern clause, int envLength, List<Node> varList) {
		...
		RETEClauseFilter ret = new RETEClauseFilter(packed, packedArgs);
		ret.id = clause.toString();
		return ret;
	}
	...
	/**
	 * Helpful function. Return the node from the argument triple corresponding to
