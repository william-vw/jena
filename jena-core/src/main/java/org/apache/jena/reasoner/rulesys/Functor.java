...
/**
 * A functor comprises a functor name and a list of arguments. The arguments are
 * Nodes of any type except functor nodes (there is no functor nesting).
 * Functors play three roles in rules - in heads they represent actions
 * (procedural attachment); in bodies they represent builtin predicates; in
 * TriplePatterns they represent embedded structured literals that are used to
 * cache matched subgraphs such as restriction specifications.
 */
public class Functor implements ClauseEntry {
	 ...
	 * @param args an array of nodes defining the arguments, this will not be copied
	 *             so beware of accidental structure sharing
	 */
	 ...
	 * @param registry a table of builtins to consult to check for implementations
	 *                 of this functor when used as a rule clause
	 */
	 ...
	 * @return true if the functor has an implementation and that implementation
	...
	/**
	 * Return a new Node array containing the bound versions of this Functor's
	 * arguments
	 */
	...
	/**
	 * Returns whether this is a transitional functor, i.e., which needs to be
	 * rolled back when the rule ultimately fails. The call is delegated to the
	 * implementor (builtin).
	 */
	...
	/**
	 * In case of a transitional functor, rollback the underlying action. The call
	 * is delegated to the implementor (builtin).
	 */
	public void rollback(BindingEnvironment env) {
		implementor.rollback(env);
	}
	...
	/**
	 * Compare Functors, taking into account variable indices. The equality function
	 * ignores differences between variables.
	 */
	@Override
