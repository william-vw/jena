 ...
 * <p>
 * This represents the rule specification but most engines will compile this
 * specification into an abstract machine or processing graph.
 * </p>
 * <p>
 * The rule specification comprises a list of antecendents (body) and a list of
 * consequents (head). If there is more than one consequent then a backchainer
 * should regard this as a shorthand for several rules, all with the same body
 * but with a singleton head.
 * </p>
 * <p>
 ...
 * arguments. The arguments are Nodes of any type except functor nodes (there is
 * no functor nesting). The functor name can be mapped into a registered java
 * class that implements its semantics. Functors play three roles - in heads
 * they represent actions (procedural attachement); in bodies they represent
 * builtin predicates; in TriplePatterns they represent embedded structured
 * literals that are used to cache matched subgraphs such as restriction
 * specifications.
 * </p>
 * <p>
 ...
 * </p>
 * We include a trivial, recursive descent parser but this is just there to
 * allow rules to be embedded in code. External rule syntax based on N3 and RDF
 * could be developed. The embedded syntax supports rules such as: <blockindent>
 ...
 * </blockindent> only built in namespaces are recognized as such, * is a
 * wildcard node, ?c is a variable, name(node ... node) is a functor, (node node
 * node) is a triple pattern, [..] is an embedded rule, commas are ignore and
 * can be freely used as separators. Functor names may not end in ':'.
 * </p>
 */
...
// accessors
	...
	/**
	 * Return true if the rule was written as a backward (as opposed to forward)
	 * rule.
	 */
	...
	/**
	 * Set the number of distinct variables for this rule. Used internally when
	 * cloing rules, not normally required.
	 */
	...
	/**
	 * Instantiate a rule given a variable binding environment. This will clone any
	 * non-bound variables though that is only needed for trail implementations.
	 */
	 ...
	 */
	private ClauseEntry[] cloneClauseArray(ClauseEntry[] clauses, Map<Node_RuleVariable, Node> vmap,
	...
	private ClauseEntry cloneClause(ClauseEntry clause, Map<Node_RuleVariable, Node> vmap, BindingEnvironment env) {
		if (clause instanceof TriplePattern) {
			...
			return new TriplePattern(cloneNode(tp.getSubject(), vmap, env), cloneNode(tp.getPredicate(), vmap, env),
	...
	/**
	 * Returns false for rules which can affect other rules non-monotonically
	 * (remove builtin or similar) or are affected non-monotonically (involve
	 * negation-as-failure).
	 */
	...
	public String toString() {
		...
		String connSym = (isTransactional ? "&" : ",");
		if (isBackward) {
			appendToString(head, connSym, buff);
			buff.append("<- ");
			appendToString(body, connSym, buff);
		} else {
			appendToString(body, connSym, buff);
			buff.append("-> ");
			appendToString(head, connSym, buff);
	...
	private void appendToString(ClauseEntry[] entries, String connSym, StringBuilder buff) {
		for (int i = 0; i < entries.length; i++) {
			if (i > 0)
				buff.append(" ").append(connSym).append(" ");
			buff.append(PrintUtil.print(entries[i]));
		}
	}

	/**
	 * Print a short description of the rule, just its name if it has one, otherwise
	 * the whole rule description.
	 */
...
// parser access
	...
	/**
	 * Parse a string as a rule, using the default BuiltinRegistry to resolve
	 * functor names
	 *
	 ...
	 * definitions (@prefix) and rule inclusion commands (@include). Returns a
	 * parser which is bound to the stripped source text with associated prefix and
	 * rule inclusion definitions.
	 */
	...
	/**
	 * Helper function find a URI argument in the current string, optionally
	 * surrounded by matching <>.
	 */
	...
	/**
	 * Helper function to return the next whitespace delimited argument from the
	 * string
	 */
	...
	/**
	 * Helper function to return the remainder of the line after stripping off the
	 * next whitespace delimited argument from the string
	 */
	...
	/**
	 * Helper function - find index of next whitespace or non white after the start
	 * index.
	 */
...
// parser support
	...
	/**
	 * Inner class which provides minimalist parsing support based on tokenisation
	 * with depth 1 lookahead. No sensible error reporting on offer. No embedded
	 * spaces supported.
	 */
	public static class Parser {
		...
		private static final char CLASSIC_CONJ_SEP = ',';
		...
		Parser(String source, BuiltinRegistry registry) {
			String separators = "()[] \t\n\r";
			separators += CLASSIC_CONJ_SEP;
			separators += SERIAL_CONJ_SEP;

			stream = new Tokenizer(source, separators, "'\"", true);
			lookahead = null;
		...
		/**
		 * Find the variable index for the given variable name and return a
		 * Node_RuleVariable with that index.
		 */
		...
		Node parseNode(String token) {
			if (token.startsWith("?")) {
				...
				// Dropped support for anon wildcards until the implementation is better
				// resolved
...
//                return Node_RuleVariable.WILD;
				...
				if (exp == token) {
					...
					if (prefix.equals("http") || prefix.equals("urn") || prefix.equals("file") || prefix.equals("ftp")
							|| prefix.equals("mailto")) {
				...
				if (peekToken().startsWith("^^")) {
					...
					if (dtURI.indexOf(':') != -1) {
						// Thanks to Steve Cranefield for pointing out the need for prefix expansion
						// here
		 ...
		 * Parse a rule, terminated by a "]" or "." character.
		 * 
		 * @param retainVarMap set to true to ccause the existing varMap to be left in
		 *                     place, which is required for nested rules.
		 */
		private Rule doParseRule(boolean retainVarMap) {
			try {
				...
				if (conjState == SERIAL_CONJ_STATE && head.size() > 1)
					throw new ParserException("Only one head term is allowed for serial conjunction", this);
				...
				r.isTransactional = (conjState == SERIAL_CONJ_STATE);
				return r;
			} catch (NoSuchElementException e) {
	...
	/**
	 * Compare clause entries, taking into account variable indices. The equality
	 * function ignores differences between variables.
	 */
	@Override
