...
/**
 * Dummy implementation of the Builtin interface that specific implementations
 * can inherit from.
 */
public abstract class BaseBuiltin implements Builtin {
	...
	public void checkArgs(int length, RuleContext context) {
		...
		if (expected > 0 && expected != length) {
			throw new BuiltinException(this, context,
					"builtin " + getName() + " requires " + expected + " arguments but saw " + length);
		}
	}
	...
	/**
	 * Returns false if this builtin is non-monotonic. This includes non-monotonic
	 * checks like noValue and non-monotonic actions like remove/drop. A
	 * non-monotonic call in a head is assumed to be an action and makes the overall
	 * rule and ruleset non-monotonic. Most JenaRules are monotonic deductive
